...
fastify.get("/bgdata4", (req, reply) => {
  var urls = ["/api/v2/properties/iob/entries.json?token=", 
...
fastify.get("/bgdex", function (request, reply) {
  ...
  var avg=null;
  var tir=[0,0,0];
  client.getEstimatedGlucoseValues({maxCount: 288, minutes: 1440})
    .then((data)=>
    {
      ...
      for (var k=data.length-1;k>=0;k--){
        if (data[k].timestamp>=oneDayPrior){
          accum+=data[k].mgdl;
          cnt++;
          if (data[k].mgdl<=request.query.tu && data[k].mgdl>=request.query.tl) {
            tir[0]+=5;
          }
          else if (data[k].mgdl<=request.query.du && data[k].mgdl>=request.query.dl) {
            tir[1]+=5;
          }
          else {
            tir[2]+=5;
  ...
  .catch((error) => {
    ...
    reply.status(error.statusCode).send({ code: error.statusCode, error: error.error}); // temporary  
  });
});
...
fastify.get("/bgdex1", function (request, reply) {
  ...
  var delta=null;
  var sgvTs=null;
  client.getEstimatedGlucoseValues({maxCount: 2, minutes: 60})
    .then((data)=>
    {
      //console.log(data.length);
      //console.log(data);
...
fastify.get("/bgdual", (request, reply) => {  
  ...
  let urls = [srv1+"/api/v1/entries.json?count=288&token="+request.query.key1, 
  ...
  Promise.all(promises).then((data) => {
    ...
    for (var i=0;i<4;i++){ // for each data type
      if (promises[i*2].response.statusCode==200){
        let x=[JSON.parse(promises[i*2].response.body), JSON.parse(promises[i*2+1].response.body)];
        for (var j=0;j<2;j++){ // for each server
          switch (i){
            case 0: // BG values
              if (x[j][0]!=null){
                sgv[j]=(x[j][0].sgv);
              ...
              for (var k=x[j].length-1;k>=0;k--){
                if (x[j][k].date>=oneDayPrior){
                  accum+=x[j][k].sgv;
                  cnt++;
                  if (x[j][k].sgv<=request.query.tu && x[j][k].sgv>=request.query.tl) {
                    tir[j][0]+=5;
                  }
                  else if (x[j][k].sgv<=request.query.du && x[j][k].sgv>=request.query.dl) {
                    tir[j][1]+=5;
                  }
                  else {
                    tir[j][2]+=5;
            ...
            case 1: // IOB
              if (x[j].iob!=null){
                iob[j]=(x[j].iob.iob);
                iobTs[j]=(x[j].iob.mills!=null ? Math.floor(x[j].iob.mills/1000):epochTime() );
              }
              else{
                iob[j]=(null);
                iobTs[j]=(null);
            ...
            case 2: // COB
              if (x[j].cob!=null){
                cob[j]=(x[j].cob.cob);
                cobTs[j]=(x[j].cob.mills!=null ? Math.floor(x[j].cob.mills/1000):epochTime() );
              }
              else{
                cob[j]=(null);
                cobTs[j]=(null);
...
fastify.get("/bgdex2", (request, reply) => {  
  ...
  var promises = [];
  promises.push(client1.getEstimatedGlucoseValues({maxCount: 288, minutes: 1440}));
  promises.push(client2.getEstimatedGlucoseValues({maxCount: 288, minutes: 1440}));
  Promise.all(promises).then((data) => {
    // data = [promise1,promise2]
    for (var i=0;i<2;i++){ // for each person
      if (data[i]!=null){
        sgv[i]=data[i][0].mgdl;
        ...
        for (var k=endIdx;k>=0;k--){
          let elem=[Math.floor(data[i][k].timestamp/1000),data[i][k].mgdl];
          h.push(elem);
        }
        hist[i]=h;
      } 
      ...
      for (var k=data[i].length-1;k>=0;k--){
        if (data[i][k].timestamp>=oneDayPrior){
          accum+=data[i][k].mgdl;
          cnt++;
          if (data[i][k].mgdl<=request.query.tu && data[i][k].mgdl>=request.query.tl) {
            tir[i][0]+=5;
          }
          else if (data[i][k].mgdl<=request.query.du && data[i][k].mgdl>=request.query.dl) {
            tir[i][1]+=5;
          }
          else {
            tir[i][2]+=5;
...
*/
fastify.get("/bgllu", function (req, reply) {
  ...
  request2(options)
      .then(function (parsedBody) {
        ...
        */
        if (parsedBody.data && parsedBody.data.redirect != null){
      ...
      .then(function (parsedBody) {
        ...
        if ([401, 403, 429, 476].includes(parsedBody.status)) {
          return handleLluError(reply, parsedBody, 2);
        }
        
        if (!parsedBody.data || !parsedBody.data.user || !parsedBody.data.authTicket) {
          return handleLluError(reply, parsedBody, 2);
        }
      ...
      .then (function (parsedBody){
        ...
        if (!Array.isArray(parsedBody.data) || parsedBody.data.length === 0) {
      ...
      .then (function (parsedBody){
        ...
        if (!parsedBody.data.connection || !parsedBody.data.connection.glucoseMeasurement) {
          return reply.code(404).send({
            success: false,
        ...
        var dir=getTrendDesc(meas.TrendArrow);
        // convert FactoryTimestamp (UTC) to an epoch time value
        var sgvTs=epochTimeD(new Date(meas.FactoryTimestamp));
        var hist=[];
        const addHist=req.query.noHist==null;
        var delta;
        ...
        if (parsedBody.data.graphData.length>0){
          ...
          for (var j=0;j<parsedBody.data.graphData.length;j++){
            ...
            var ts=epochTimeD(new Date(parsedBody.data.graphData[j].FactoryTimestamp));
            if (addHist && nowTs-ts<7500){  // 2h+5min margin
              let elem=[ts, parsedBody.data.graphData[j].ValueInMgPerDl];
              hist.push(elem);              
            }
            if (ts==sgvTs && j>0){  // if latest value
              // calculate delta from previous value
              delta=parsedBody.data.graphData[j].ValueInMgPerDl-parsedBody.data.graphData[j-1].ValueInMgPerDl;
            }            
          }          
        }
        resp={
      ...
      .catch(function (err) {
        console.log("ERROR!");
        console.error(err);
        const code = typeof err.statusCode === "number" && err.statusCode > 0 ? err.statusCode : 500;
...
fastify.get("/bgllu2", function (req, reply) {
  ...
  request2(options)
      ...
      .then(function (parsedBody) {
        ...
        let userId=parsedBody.data.user.id;
        token=parsedBody.data.authTicket.token;
        accountId = crypto.createHash("sha256").update(parsedBody.data.user.id).digest("hex");
        let options2={
          method: 'GET',
          uri: server+'/llu/connections',
          headers: {
      ...
      .then (function (parsedBody){
        ...
        if (parsedBody.data.graphData.length>0){
          ...
          for (var j=0;j<parsedBody.data.graphData.length;j++){
            // limit to data 2 hours old
            var ts=epochTimeD(new Date(parsedBody.data.graphData[j].FactoryTimestamp));
            if (nowTs-ts<7500){  // 2h+5min margin
              let elem=[ts, parsedBody.data.graphData[j].ValueInMgPerDl];
              hist[0].push(elem);              
            }
            if (ts==sgvTs[0] && j>0){  // if latest value
              // calculate delta from previous value
              delta[0]=parsedBody.data.graphData[j].ValueInMgPerDl-parsedBody.data.graphData[j-1].ValueInMgPerDl;
            }      
          }       
        }
      ...
      .then (function (parsedBody){
        ...
        if (parsedBody.data.graphData.length>0){
          ...
          for (var j=0;j<parsedBody.data.graphData.length;j++){
            ...
            if (ts==sgvTs[1] && j>0){  // if latest value
              // calculate delta from previous value
              delta[1]=parsedBody.data.graphData[j].ValueInMgPerDl-parsedBody.data.graphData[j-1].ValueInMgPerDl;
            }      
          }       
        }
        resp={
...
fastify.get("/bgllutest", function (req, reply) {
  ...
  request(options, function (error, response, body) {
      resp={success : false, step: 1, code: response.statusCode,
          r: response
      };
